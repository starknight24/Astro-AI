...
import { Upload, BookOpen, Brain, Send, Loader2, Sparkles, AlertCircle, ChevronRight, Check, Copy } from "lucide-react";
...
const PRELOADED_PAPERS: ResearchPaper[] = [
  {
    id: "jwst-l2",
    title: "Optical Alignment and Early Calibration of the James Webb Space Telescope at the Sun-Earth L2 Lagrange Point",
    authors: "Dr. Elena Vance, Senior Astrophysicist at Space Telescope Science Institute (STScI)",
    abstract: "This paper reports the initial optical alignment alignment and mid-infrared calibration results of the JWST primary mirror arrays following deployment to its halo orbit around Sun-Earth L2. We detail the wavefront sensing algorithms and multi-step phase retrieval processes utilized to align the 18 hexagonal beryllium segment mirrors down to diffraction-limited performance at 2 microns. Alignment telemetry indicates stable rms wavefront errors < 150 nm over 48-hour observational cycles.",
    equations: "$$WFE_{rms} = \\sqrt{\\frac{1}{A}\\iint_A [W(x,y) - \\bar{W}]^2 dxdy}$$ where $W(x,y)$ represents the wavefront phase map and $A$ is the aperture. Secondary mirror corrective impulse equation is given by: $$\\Delta z = C \\cdot \\vec{\\phi}_k$$",
    methodology: "Using primary mirror segment actuator controls in 6 degrees of freedom. Alignment checks performed via phase retrieval on the Near Infrared Camera (NIRCam) focal plane arrays. Hexagonal focus corrections deployed sequentially over a 14-day timeline.",
    findings: "The primary mirror array reached diffraction-limited alignment with total wavefront error measuring 137 nm, significantly exceeding the mission's pre-launch requirement of 150 nm. High-contrast coronagraph calibration confirms 10^-5 contrast levels at 1 arcsecond.",
  ...
  {
    id: "hohmann-multi",
    title: "Optimizing Hohmann Transfer Trajectories with Multi-Impulse Mid-Course Correction Engines",
    authors: "Prof. Arthur Pendelton, Department of Aerospace Engineering, MIT",
    abstract: "Traditional Hohmann transfer orbits assume two singular instantaneous burns at periapsis and apoapsis. This paper evaluates the delta-V efficiency gains achieved by integrating small mid-course corrections during low-thrust chemical transfers. Numerical simulations of transfers from Low Earth Orbit (LEO) to Geostationary Transfer Orbit (GTO) indicate that adding a third correction burn at the transfer orbit's true anomaly $\\theta = 90^\\circ$ minimizes inclination correction errors by up to 12.4% under severe launch vector dispersion.",
    equations: "$$\\Delta v_{total} = \\Delta v_1 + \\Delta v_2 + \\sum_{i=3}^n \\Delta v_{mid}$$ where $$\\Delta v_1 = v_{tx1} - v_{c1} = \\sqrt{\\mu \\left(\\frac{2}{r_1} - \\frac{1}{a_{tx}}\\right)} - \\sqrt{\\frac{\\mu}{r_1}}$$",
    methodology: "Conducted Monte Carlo simulations with 10,000 runs representing launch trajectory dispersion. Engine thrust models implemented using standard Runge-Kutta 4th-order orbital integration parameters with customized gravitational modeling.",
    findings: "While total theoretical delta-v remains constrained by standard vis-viva parameters, mid-course corrections decrease orbit correction margins on arrival at GEO. High-eccentricity solar radiation pressure perturbation was successfully mitigated.",
    contentSnippet: "Two-impulse Hohmann transfers assume absolute thrust duration equals zero (impulsive approximation). In actual chemical spacecraft execution, finite burn times lead to gravity losses. Mid-course correction maneuvers (MCCs) are scheduled using true anomaly milestones. For high inclination changes, conducting inclination change during the second burn at apoapsis (where speed is minimal) is optimal. Launch dispersion error covariance matrices demonstrate that without active mid-course adjustments, typical navigation systems miss GEO insertion windows by an average of 420 km."
  },
  {
    id: "m-dwarf-bio",
    title: "Spectroscopic Biosignature Analysis and Atmosphere Modeling of Exoplanets Orbiting M-Dwarf Systems",
    authors: "Sarah J. Thorne, Department of Astronomy and Astrobiology, York University",
    abstract: "M-dwarf stars are the most common hosts for rocky exoplanets, but their stellar flares present astrobiological hazards. This research models the atmospheric ozone and methane concentrations on a terrestrial exoplanet orbiting within the habitable zone of an active M-dwarf star. We simulate high-resolution transmission spectra (0.3 to 5 microns) to assess the detectability of simultaneous $O_3$ and $CH_4$ biosignatures using next-generation space telescope coronagraphs.",
    equations: "$$I(\\lambda) = I_0(\\lambda) \\cdot e^{-\\tau(\\lambda)}$$ where $\\tau(\\lambda) = \\int \\rho(z)\\sigma(\\lambda, z)dz$ is the optical depth along the transmission path $z$ with local atmospheric density $\\rho$.",
    methodology: "1D radiative-convective-photochemical modeling coupled with synthetic transmission spectra generators. Stellar UV flares modeled based on observations of Proxima Centauri.",
    findings: "Simultaneous detection of methane ($CH_4$) at 1.66 microns and ozone ($O_3$) at 9.6 microns represents a robust astrobiological biosignature. Flare events enhance ozone regeneration via high-energy photolysis.",
    contentSnippet: "Terrestrial exoplanets around M-dwarf stars are tidally locked, meaning they present the same face to the host star. This produces severe global temperature gradients. Global atmospheric circulation model (GCM) simulations indicate that atmospheric winds redistribute thermal energy to prevent nightside condensation. Spectroscopic observations during transit probe the exoplanet's atmosphere. Chemical networks show that methane, when combined with high ozone levels, cannot exist in thermodynamic equilibrium without active, biological surface emissions."
  }
];

export default function PaperRag({ onActivityAdded }: PaperRagProps) {
  const [selectedPaper, setSelectedPaper] = useState<ResearchPaper>(PRELOADED_PAPERS[0]);
  const [papersList, setPapersList] = useState<ResearchPaper[]>(PRELOADED_PAPERS);
  ...
  const handleSelectPaper = (paper: ResearchPaper) => {
    ...
    onActivityAdded("chat", `Loaded paper for RAG analysis: ${paper.title.substring(0, 30)}...`);
  };
  ...
  const processUploadedFile = async (file: File) => {
    ...
    try {
      ...
      reader.onload = async (event) => {
        const text = (event.target?.result as string) || `This is parsed research data about space engineering on file ${file.name}`;
        ...
        onActivityAdded("chat", `Uploaded and RAG-processed custom paper: ${file.name}`);
      };
      reader.readAsText(file);
    } catch (err: any) {
      alert(`Error reading file: ${err.message || "Failed to parse document"}`);
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleSendRagChat = async () => {
    ...
    try {
      const response = await fetch("/api/papers/chat", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          paperText: selectedPaper.contentSnippet + " " + (selectedPaper.abstract || ""),
      ...
      onActivityAdded("chat", `Queried paper RAG about: "${msg.substring(0, 25)}..."`);
    } catch (err: any) {
      setChatMessages(prev => [
        ...prev,
        {
          id: (Date.now() + 1).toString(),
          role: "model",
          text: `⚠️ **RAG Query Error**: ${err.message || "Failed to synchronize grounding models."}`,
          timestamp: new Date()
        }
      ]);
    } finally {
  ...
  const renderLaTeX = (text?: string) => {
    ...
    return blocks.map((block, idx) => {
      if (block.startsWith("$$") && block.endsWith("$$")) {
        return (
          <div key={idx} className="my-2 p-2.5 bg-slate-900 border border-emerald-500/20 rounded font-mono text-emerald-400 overflow-x-auto text-center text-xs">
            {block.slice(2, -2)}
          </div>
        );
      }
        ...
        <span key={idx}>
          {inlines.map((sub, sIdx) => {
            if (sub.startsWith("$") && sub.endsWith("$")) {
              return (
                <code key={sIdx} className="px-1 py-0.5 mx-0.5 bg-slate-900 border border-cyan-500/20 text-cyan-400 rounded font-mono text-xs">
                  {sub.slice(1, -1)}
                </code>
              );
            }
    ...
    <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 h-[calc(100vh-8rem)]">
      ...
      <div className="xl:col-span-3 flex flex-col gap-4 border border-slate-800 rounded-2xl bg-slate-950/60 p-4 overflow-y-auto">
        <div className="flex items-center gap-2 pb-2 border-b border-slate-800">
          ...
          <h3 className="font-display font-semibold text-white">Space Library</h3>
        </div>
        ...
        <div className="flex-1 flex flex-col gap-2 overflow-y-auto max-h-[300px] xl:max-h-none">
          {papersList.map(p => (
            ...
            >
              <span className="font-bold text-slate-100 line-clamp-2">{p.title}</span>
              <span className="text-[10px] opacity-80 italic line-clamp-1">By: {p.authors}</span>
            </button>
          ))}
        </div>
        ...
        >
          {uploading ? (
            <div className="flex flex-col items-center gap-2 py-2">
              ...
              <span className="text-xs text-indigo-300">Extracting PDF text...</span>
            </div>
          ) : (
            <label className="cursor-pointer flex flex-col items-center gap-1.5 py-1">
              <Upload className="w-5 h-5 text-slate-400" />
              <span className="text-xs font-semibold text-slate-300">Add Scientific PDF / TXT</span>
              <span className="text-[10px] text-slate-500 font-mono uppercase">Drag-and-drop or click</span>
              <input
                type="file"
                accept=".pdf,.txt"
      ...
      <div className="xl:col-span-5 flex flex-col border border-slate-800 rounded-2xl bg-slate-950/60 p-5 overflow-y-auto">
        <div className="flex items-center gap-2 pb-3 border-b border-slate-800 mb-4 justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-display font-semibold text-white">Auto-Extracted Syntheses</h3>
          </div>
        ...
        <div className="space-y-5 text-sm text-slate-300">
          <div>
            <h4 className="font-display font-bold text-white text-xs uppercase tracking-wider font-mono text-indigo-300 mb-1.5">Abstract Summary</h4>
          ...
          <div>
            <h4 className="font-display font-bold text-white text-xs uppercase tracking-wider font-mono text-indigo-300 mb-1.5">Core Physics Equations</h4>
          ...
          <div>
            <h4 className="font-display font-bold text-white text-xs uppercase tracking-wider font-mono text-indigo-300 mb-1.5">Methodology & Verification</h4>
          ...
          <div>
            <h4 className="font-display font-bold text-white text-xs uppercase tracking-wider font-mono text-indigo-300 mb-1.5">Key Findings & Accomplishments</h4>
      ...
      <div className="xl:col-span-4 flex flex-col border border-slate-800 rounded-2xl bg-slate-950/40 overflow-hidden shadow-xl">
        <div className="p-4 border-b border-slate-800 bg-slate-900/70 flex items-center gap-2 shrink-0">
          ...
          <div>
            <h3 className="font-display font-semibold text-white text-xs">Grounding Peer Reviewer</h3>
            <p className="text-[10px] text-slate-400 font-mono">Anchored specifically to this paper</p>
          </div>
        </div>
