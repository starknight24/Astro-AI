...
import { Globe, MapPin, Search, ChevronRight, Activity, Calendar, ShieldAlert, Eye } from "lucide-react";
...
const SAMPLE_ASTEROIDS: Asteroid[] = [
  { id: "1", name: "(2026 AM1)", diameterMin: 45, diameterMax: 98, velocity: 41200, missDistance: 3.2, isHazardous: false, date: "2026-06-26" },
  { id: "2", name: "(2026 BF3)", diameterMin: 120, diameterMax: 270, velocity: 68500, missDistance: 1.8, isHazardous: true, date: "2026-06-26" },
  { id: "3", name: "Apophis (99942)", diameterMin: 340, diameterMax: 370, velocity: 110200, missDistance: 0.08, isHazardous: true, date: "2026-06-27" },
  { id: "4", name: "(2026 CG9)", diameterMin: 12, diameterMax: 28, velocity: 22400, missDistance: 8.4, isHazardous: false, date: "2026-06-28" },
  { id: "5", name: "(2026 DH2)", diameterMin: 70, diameterMax: 150, velocity: 53100, missDistance: 4.1, isHazardous: false, date: "2026-06-28" }
];

// Mars Rover Images Fallbacks
const ROVER_IMAGES = [
  { rover: "Perseverance", camera: "Mastcam-Z", url: "https://photojournal.jpl.nasa.gov/jpeg/PIA24484.jpg", caption: "Jezero Crater delta panoramas taken by Perseverance." },
  { rover: "Curiosity", camera: "Navcam", url: "https://photojournal.jpl.nasa.gov/jpeg/PIA19803.jpg", caption: "Gale Crater Mt. Sharp layers, explored by Curiosity." },
  { rover: "Perseverance", camera: "Front Hazcam", url: "https://photojournal.jpl.nasa.gov/jpeg/PIA24424.jpg", caption: "Perseverance's first raw hazcam view of Jezero's floor." },
  { rover: "Curiosity", camera: "Mast Camera", url: "https://photojournal.jpl.nasa.gov/jpeg/PIA16226.jpg", caption: "Self-portrait of Curiosity Rover at 'Rocknest' site." }
];

export default function NasaExplorer({ onActivityAdded }: NasaExplorerProps) {
  const [tab, setTab] = useState<"apod" | "asteroids" | "rovers" | "iss">("apod");

  // APOD
  const [apod, setApod] = useState<APODData>(APOD_FALLBACK);
  const [loadingApod, setLoadingApod] = useState(false);

  // Asteroid search state
  const [asteroids, setAsteroids] = useState<Asteroid[]>(SAMPLE_ASTEROIDS);
  ...
  // ISS Tracker state
  const [issPos, setIssPos] = useState({ lat: 21.34, lon: -42.88, alt: 421.5, vel: 27580 });
    ...
    <div className="flex flex-col h-[calc(100vh-8rem)] bg-slate-950/60 border border-slate-800 rounded-2xl overflow-hidden shadow-2xl backdrop-blur-md">
      ...
      <div className="px-6 py-3 border-b border-slate-800 bg-slate-900/70 flex flex-wrap justify-between items-center gap-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-display font-semibold text-white">NASA Space Exploration Databases</h2>
        </div>
        ...
        {tab === "apod" && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-6xl mx-auto items-center">
            ...
            <div className="flex flex-col gap-4">
              ...
              <h3 className="font-display font-bold text-2xl text-white tracking-tight leading-snug">{apod.title}</h3>
              <p className="text-sm text-slate-300 leading-relaxed max-h-[250px] overflow-y-auto pr-2">{apod.explanation}</p>
        ...
        {tab === "asteroids" && (
          <div className="flex flex-col gap-5 max-w-5xl mx-auto">
            <div className="flex flex-col sm:flex-row gap-3 justify-between items-start sm:items-center">
              <div>
                <h3 className="font-display font-semibold text-lg text-white">Asteroid Tracker (NeoWs API)</h3>
                <p className="text-xs text-slate-400">Near-Earth Asteroid objects with orbit risk analysis</p>
              </div>
              ...
              {filteredAsteroids.map(ast => (
                ...
                >
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-sm font-bold text-slate-100">{ast.name}</span>
                      {ast.isHazardous && (
                    ...
                    <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-xs font-mono text-slate-400">
                      <div>Diameter: <strong className="text-slate-200">{ast.diameterMin}-{ast.diameterMax} m</strong></div>
                      <div>Speed: <strong className="text-slate-200">{ast.velocity.toLocaleString()} km/h</strong></div>
                      <div>Miss distance: <strong className="text-cyan-400">{ast.missDistance} Lunar Dist.</strong></div>
                      <div>Observation: <strong className="text-slate-200">{ast.date}</strong></div>
                    </div>
                  </div>
                  ...
                  <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 text-center shrink-0">
                    ...
                    <p className={`text-md font-mono font-bold ${ast.isHazardous ? "text-rose-400 animate-pulse" : "text-emerald-400"}`}>
        ...
        {tab === "rovers" && (
          <div className="flex flex-col gap-6 max-w-5xl mx-auto">
            <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
              <div>
                <h3 className="font-display font-semibold text-lg text-white">Mars Exploratory Rover Cameras</h3>
                <p className="text-xs text-slate-400">Visual feeds archived from JPL mission logs</p>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {filteredRoverImages.map((img, idx) => (
                <div key={idx} className="bg-slate-900/50 border border-slate-800 rounded-2xl overflow-hidden flex flex-col gap-3 group shadow">
                  ...
                  <p className="px-4 pb-4 text-xs text-slate-400 leading-relaxed font-sans">{img.caption}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 max-w-5xl mx-auto items-center">
            ...
            <div className="md:col-span-7 flex flex-col gap-3 items-center">
              <div className="relative w-full aspect-[2/1] bg-slate-950 border border-slate-800 rounded-xl overflow-hidden shadow-inner flex items-center justify-center space-grid">
                ...
                <svg className="w-full h-full" viewBox={`0 0 ${mapWidth} ${mapHeight}`}>
                  {/* Grid lines */}
                  <line x1="0" y1="80" x2="320" y2="80" stroke="rgba(148, 163, 184, 0.08)" />
                  <line x1="160" y1="0" x2="160" y2="160" stroke="rgba(148, 163, 184, 0.08)" />
                ...
                <div className="absolute top-3 left-3 bg-slate-950/95 border border-slate-800 p-2 rounded text-[10px] font-mono text-cyan-400 flex items-center gap-1.5">
                  <Activity className="w-3.5 h-3.5 text-cyan-500 animate-pulse" /> Telemetry Stream: LIVE
                </div>
              </div>
              <span className="text-[10px] text-slate-500 font-mono tracking-wider">ISS ORBITAL ECCENTRICITY: ~0.00029 | INCLINATION: 51.64°</span>
            </div>
            ...
            <div className="md:col-span-5 flex flex-col gap-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-display font-semibold text-white">ISS Real-time Space Tracker</h3>
              </div>
              
              <p className="text-xs text-slate-400 leading-relaxed">
                Visualizing coordinates, velocity, and altitude tracking for the International Space Station. The ISS travels at roughly 7.66 km/s, completing an orbit every 92.9 minutes.
              </p>

              <div className="grid grid-cols-2 gap-4 font-mono text-xs">
                <div className="p-4 bg-slate-950 border border-slate-800 rounded-xl">
                  <span className="text-[10px] text-slate-500 uppercase block">Latitude</span>
                  <strong className="text-md text-white mt-1 block">{issPos.lat > 0 ? `${issPos.lat}° N` : `${Math.abs(issPos.lat)}° S`}</strong>
                </div>
                <div className="p-4 bg-slate-950 border border-slate-800 rounded-xl">
                  <span className="text-[10px] text-slate-500 uppercase block">Longitude</span>
                  <strong className="text-md text-white mt-1 block">{issPos.lon > 0 ? `${issPos.lon}° E` : `${Math.abs(issPos.lon)}° W`}</strong>
                </div>
                <div className="p-4 bg-slate-950 border border-slate-800 rounded-xl">
                  <span className="text-[10px] text-slate-500 uppercase block">Altitude</span>
                  <strong className="text-md text-cyan-400 mt-1 block">{issPos.alt} km</strong>
                </div>
                <div className="p-4 bg-slate-950 border border-slate-800 rounded-xl">
                  <span className="text-[10px] text-slate-500 uppercase block">Speed</span>
                  <strong className="text-md text-cyan-400 mt-1 block">{(issPos.vel).toLocaleString()} km/h</strong>
                </div>
              </div>
            </div>
