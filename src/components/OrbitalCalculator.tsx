...
import { Orbit, Play, Pause, RotateCcw, Save, Flame, Compass, ChevronRight, HelpCircle, Activity } from "lucide-react";
...
export default function OrbitalCalculator({ onActivityAdded, onSaveNote }: OrbitalCalculatorProps) {
  ...
  // Specific Orbital Energy Inputs
  const [specVelocity, setSpecVelocity] = useState<number>(7.67);
  const [specRadius, setSpecRadius] = useState<number>(6771);
  ...
  const [hohmannPhase, setHohmannPhase] = useState<"idle" | "burn1" | "coasting" | "burn2" | "circular">("idle");
  ...
  // Sphere of Influence calculation relative to Sun (approximations)
  const soi_approx = body === "Earth" ? 925000 : body === "Mars" ? 577000 : 66000;
  ...
  // Specific energy for custom values
  const custom_specific_energy = (Math.pow(specVelocity, 2) / 2) - (bodyData.mu / specRadius);
      ...
      if (isPlaying) {
        ...
        if (isHohmannAnimating) {
          if (hohmannPhase === "coasting") {
            ...
            const hohmannTofMultiplier = (Math.PI / tof_sec) * (simSpeed / 60) * 3;
            hohmannAngleRef.current = hohmannAngleRef.current + hohmannTofMultiplier;
            ...
            const circVelocity = (2 * Math.PI) / (2 * Math.PI * Math.sqrt(Math.pow(r2, 3) / bodyData.mu));
            const circStep = circVelocity * (simSpeed / 60) * 1.5;
            hohmannAngleRef.current = (hohmannAngleRef.current + circStep) % (2 * Math.PI);
            setHohmannSimAngle(hohmannAngleRef.current);
          }
        }
      }
  ...
  }, [isPlaying, simSpeed, period_sec, isHohmannAnimating, hohmannPhase, tof_sec, r2, bodyData.mu]);

  const handleStartHohmannTransfer = () => {
    onActivityAdded("calculator", `Hohmann Transfer Simulated: ${depAltitude}km to ${arrAltitude}km`);
...
Time of Flight: ${(tof_sec / 3600).toFixed(2)} hours (${(tof_sec/60).toFixed(1)} mins)`;
  ...
  if (hohmannPhase === "burn1" || hohmannPhase === "idle") {
    ...
    const rx = (a_tx * (1 - Math.pow(hohmannEccentricity, 2))) / (1 + hohmannEccentricity * Math.cos(hohmannSimAngle));
      ...
      <div className="lg:col-span-7 flex flex-col gap-6">
        ...
        <div className="p-6 bg-slate-900/60 border border-slate-800 rounded-2xl flex flex-col gap-5 backdrop-blur-md">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-display font-semibold text-white">Orbital Mechanics Setup</h3>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {/* Body Select */}
            <div>
              <label className="block text-xs font-medium text-slate-400 mb-1.5 uppercase tracking-wider font-mono">Central Celestial Body</label>
              <select
                value={body}
            ...
            {/* Circular altitude */}
            <div>
              <label className="block text-xs font-medium text-slate-400 mb-1.5 uppercase tracking-wider font-mono">Circular Alt. (km)</label>
              <input
                type="number"
                value={altitude}
                onChange={(e) => setAltitude(Math.max(10, parseInt(e.target.value) || 0))}
            ...
            <div className="flex flex-col justify-end">
              <span className="text-[10px] text-slate-500 font-mono">CELESTIAL CONSTANTS:</span>
          ...
          <div className="bg-slate-950/80 rounded-xl p-4 border border-slate-800 grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
            <div>
              <p className="text-[10px] text-slate-500 font-mono uppercase tracking-wider">Orbital Speed</p>
              <p className="text-lg font-bold text-emerald-400 font-mono mt-1">{v_c.toFixed(4)} <span className="text-xs font-normal text-slate-400">km/s</span></p>
            </div>
            <div>
              <p className="text-[10px] text-slate-500 font-mono uppercase tracking-wider">Escape velocity</p>
              <p className="text-lg font-bold text-cyan-400 font-mono mt-1">{escape_v.toFixed(4)} <span className="text-xs font-normal text-slate-400">km/s</span></p>
            </div>
            <div>
              <p className="text-[10px] text-slate-500 font-mono uppercase tracking-wider">Orbital Period</p>
              <p className="text-lg font-bold text-amber-400 font-mono mt-1">
                {period_min > 120 ? `${(period_min / 60).toFixed(2)} h` : `${period_min.toFixed(1)} m`}
              </p>
            </div>
            <div>
              <p className="text-[10px] text-slate-500 font-mono uppercase tracking-wider">Specific Energy</p>
              <p className="text-lg font-bold text-rose-400 font-mono mt-1">{spec_energy.toFixed(2)} <span className="text-xs font-normal text-slate-400">MJ/kg</span></p>
            </div>
          </div>
          ...
          <div className="p-4 bg-slate-950/40 rounded-xl border border-slate-800/60 flex flex-col gap-3 text-xs text-slate-400">
            <h4 className="font-display font-medium text-slate-200">Keplerian Circular Formula Breakdown</h4>
            <div className="space-y-2 font-mono">
              <div className="flex justify-between items-center border-b border-slate-900 pb-1.5">
                <span>Semi-major axis ($r$):</span>
                <span className="text-slate-300">R + h = {bodyData.radius} + {altitude} = <strong className="text-indigo-400">{r.toFixed(1)} km</strong></span>
              </div>
              <div className="flex justify-between items-center border-b border-slate-900 pb-1.5">
                <span>Orbital velocity ($v_c$):</span>
                <span className="text-slate-300">√ (μ / r) = √ ({bodyData.mu} / {r.toFixed(1)}) = <strong className="text-emerald-400">{v_c.toFixed(4)} km/s</strong></span>
              </div>
              <div className="flex justify-between items-center">
                <span>Escape speed ($v_e$):</span>
                <span className="text-slate-300">√ (2μ / r) = √ (2 * {bodyData.mu} / {r.toFixed(1)}) = <strong className="text-cyan-400">{escape_v.toFixed(4)} km/s</strong></span>
              </div>
            </div>
        ...
        <div className="p-6 bg-slate-900/60 border border-slate-800 rounded-2xl flex flex-col gap-4 backdrop-blur-md">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-display font-semibold text-white">Hohmann Transfer Trajectory</h3>
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-slate-400 mb-1 uppercase tracking-wider font-mono">Departure Alt (km)</label>
              <input
                type="number"
                value={depAltitude}
                onChange={(e) => setDepAltitude(Math.max(10, parseInt(e.target.value) || 0))}
            ...
            <div>
              <label className="block text-xs font-medium text-slate-400 mb-1 uppercase tracking-wider font-mono">Arrival Alt (km)</label>
              <input
                type="number"
                value={arrAltitude}
                onChange={(e) => setArrAltitude(Math.max(10, parseInt(e.target.value) || 0))}
                className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-sm text-slate-100 font-mono focus:border-indigo-500 focus:outline-none"
              />
            </div>
          </div>
          ...
          <div className="bg-slate-950/80 rounded-xl p-4 border border-slate-800 grid grid-cols-2 sm:grid-cols-3 gap-4 text-center font-mono">
            <div>
              <p className="text-[10px] text-slate-500 uppercase">Departure Burn ΔV₁</p>
              <p className="text-md font-bold text-indigo-400 mt-1">{delta_v1.toFixed(4)} km/s</p>
            </div>
            <div>
              <p className="text-[10px] text-slate-500 uppercase">Arrival Burn ΔV₂</p>
              <p className="text-md font-bold text-indigo-400 mt-1">{delta_v2.toFixed(4)} km/s</p>
            </div>
            <div className="col-span-2 sm:col-span-1">
              <p className="text-[10px] text-slate-500 uppercase">Total Required ΔV</p>
              <p className="text-md font-bold text-emerald-400 mt-1">{total_delta_v.toFixed(4)} km/s</p>
            </div>
          </div>
          ...
          <div className="space-y-1.5 text-xs text-slate-400 font-mono bg-slate-950/40 p-3.5 rounded-xl border border-slate-800/60">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-amber-400 font-medium">{(tof_sec / 3600).toFixed(2)} hours ({(tof_sec / 60).toFixed(1)} minutes)</span>
            </div>
          </div>
          ...
          <div className="flex justify-between items-center mt-2">
            {!isHohmannAnimating ? (
              ...
              >
                <Flame className="w-4 h-4 text-white animate-pulse" /> Trigger Hohmann Transfer Animation
              </button>
            ) : (
              <button
        ...
        <div className="p-6 bg-slate-900/60 border border-slate-800 rounded-2xl flex flex-col items-center justify-between gap-4 backdrop-blur-md flex-1">
          <div className="w-full flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-display font-semibold text-white">Interactive HUD Simulator</h3>
            </div>
          ...
          <div className="relative w-full aspect-square bg-slate-950 border border-slate-800/80 rounded-xl overflow-hidden shadow-inner flex items-center justify-center space-grid">
            ...
            <div className="absolute inset-0 pointer-events-none opacity-40">
              <div className="absolute top-8 left-1/4 w-1 h-1 bg-white rounded-full animate-ping" style={{ animationDuration: "3s" }} />
            ...
            <svg className="w-full h-full max-w-[360px]" viewBox="0 0 300 300">
              ...
              <circle cx="150" cy="150" r="50" fill="none" stroke="rgba(148, 163, 184, 0.04)" strokeDasharray="3,3" />
              <circle cx="150" cy="150" r="90" fill="none" stroke="rgba(148, 163, 184, 0.04)" strokeDasharray="3,3" />
              <circle cx="150" cy="150" r="130" fill="none" stroke="rgba(148, 163, 184, 0.04)" strokeDasharray="3,3" />
              ...
              {isHohmannAnimating && (
                <>
                  {/* Dep orbit circular */}
                  <circle cx="150" cy="150" r={depSvgRadius} fill="none" stroke="rgba(99, 102, 241, 0.3)" strokeWidth="1" strokeDasharray="2,2" />
                  
                  {/* Arr orbit circular */}
                  <circle cx="150" cy="150" r={arrSvgRadius} fill="none" stroke="rgba(16, 185, 129, 0.3)" strokeWidth="1" strokeDasharray="2,2" />

                  {/* Transfer ellipse: Semi-major axis = tx_a, semi-minor axis = tx_b. Focused on Central body */}
                  {(hohmannPhase === "coasting" || hohmannPhase === "burn1") && (
                    <ellipse
              ...
              <circle cx="150" cy="150" r={planetSvgRadius * 0.75} fill="rgba(0, 0, 0, 0.15)" />

              {/* Circular satellite orbiter */}
              {!isHohmannAnimating && (
                <g>
                  <circle cx={satX} cy={satY} r="4" fill="#6366f1" className="animate-pulse" />
                  <circle cx={satX} cy={satY} r="8" fill="none" stroke="#6366f1" strokeWidth="1" opacity="0.6" className="animate-ping" style={{ animationDuration: "2s" }} />
                </g>
              )}
              ...
              {isHohmannAnimating && (
                <g>
                  <circle cx={hSatX} cy={hSatY} r="5" fill={hohmannPhase === "coasting" ? "#f59e0b" : hohmannPhase === "circular" ? "#10b981" : "#6366f1"} />
            ...
            <div className="absolute bottom-3 left-3 bg-slate-950/90 border border-slate-800 p-2.5 rounded-lg text-[10px] font-mono text-slate-400 flex flex-col gap-1">
              <div className="flex justify-between gap-4">
                <span>SIM STATE:</span>
                <span className={isPlaying ? "text-emerald-400" : "text-rose-400"}>{isPlaying ? "ACTIVE" : "PAUSED"}</span>
              </div>
              <div className="flex justify-between gap-4">
              ...
              {isHohmannAnimating && (
                <div className="flex justify-between gap-4 border-t border-slate-900 pt-1 mt-1 font-semibold">
                  ...
                  <span className="text-amber-400 uppercase">{hohmannPhase}</span>
                </div>
              )}
            </div>
            ...
            {isHohmannAnimating && hohmannPhase === "coasting" && (
              <div className="absolute top-3 left-3 bg-amber-500/10 border border-amber-500/30 px-3 py-1.5 rounded-lg font-mono text-xs text-amber-400">
                🚀 Coasting TOF Timeleft: {(hohmannTimeLeft / 60).toFixed(1)} mins
              </div>
            )}
          </div>
          ...
          <div className="w-full">
            <div className="flex justify-between items-center mb-1">
              <span className="text-xs font-medium text-slate-400 font-mono">SIMULATION TIME SLIDER</span>
              <span className="text-xs font-bold text-slate-200 font-mono">{simSpeed}x warp</span>
            </div>
            <input
              type="range"
              min="1"
